    ...
    for(let i = 0; i < rooms.length; i++) {
      if(rooms[i]._id === messages.roomId) {
        if(rooms[i].settings === undefined) {
          ...
          return (
            <div className='room-basics'>
              {friend.settings.profilePic.pic.length > 1 ? (
                <img className="user-profile" alt="profile"src={friend.settings.profilePic.pic} onClick={toggleRoomProfile}/>
              ) : null}
              {friend.settings.profilePic.pic.length === 1 ? (
          ...
          return (
            <div className="room-basics">
              {room.settings.profileURL.length > 1 ? (
                <img className="user-profile" alt="profile" src={room.settings.profileURL} onClick={toggleRoomProfile}/>
              ) : null}
              {room.settings.profileURL.length === 1 ? (
                <div className="user-profile" style={{backgroundColor: '#' + room.settings.hex}} onClick={toggleRoomProfile}>{room.settings.profileURL}</div>
              ) : null}
              <div className="room-info">
                {room.roomName}
